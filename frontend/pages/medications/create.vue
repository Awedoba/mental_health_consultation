<template>
  <div>
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold text-gray-900">Add Medication</h1>
      <NuxtLink
        to="/medications"
...
const handleSubmit = async () => {
  ...
  if (result.success) {
    showToast('success', 'Medication created successfully')
    router.push('/medications')
  } else {
    error.value = result.error || 'Failed to create medication'
    validationErrors.value = result.errors || null
    handleApiError(result.error || 'Failed to create medication')
  }
